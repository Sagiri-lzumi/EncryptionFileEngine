...
下载 EncryptionStudio_V1.0.7z 压缩包。

将其解压到任意文件夹（⚠️ 注意：必须解压，不能直接在压缩包内运行）。

进入解压后的文件夹，双击运行 EncryptionStudio_V1.0.exe。

程序会自动在同级目录下生成 Logs（日志）和 Keys（密钥）文件夹，请勿随意删除。
...
如何使用：
直接双击 EncryptionStudio_V1.0.exe 运行。

注意：首次启动可能需要等待 3-5 秒（程序正在自解压到内存），请耐心等待，不要重复点击。
程序运行后，会自动在 .exe 文件旁边生成配置文件夹。建议将程序放在一个专门的文件夹中运行，以免生成的文件显得杂乱。
---
## ✨ 核心功能 (Key Features)
